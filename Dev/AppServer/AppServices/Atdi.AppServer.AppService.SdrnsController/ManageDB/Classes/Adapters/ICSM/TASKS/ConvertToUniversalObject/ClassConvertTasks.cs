...
namespace Atdi.SDNRS.AppServer.ManageDB.Adapters
{
    ...
    public class ClassConvertTasks : IDisposable
    {
        ...
        public MeasTask[] ConvertTo_MEAS_TASKObjectsFast(List<CLASS_TASKS> objs)
        {
            const int Cn = 900;
            List<MeasTask> L_OUT = new List<MeasTask>();
            try
                ...
                logger.Trace("Start procedure ConvertTo_MEAS_TASKObjects...");
                    try
                    {
                        ...
                        foreach (CLASS_TASKS obj in objs.ToArray())
                        {
                            ...
                            {
                                int idx_YXbsSector = 0;
                                List<int> sql_YXbsSector_in = new List<int>();
                                string sql_YXbsSector = "";
                                ...
                                foreach (YXbsStation r_dt in obj.XbsStationdatform.ToArray())
                                {
                                    ...
                                    if (sql_YXbsSector_in.Count <= Cn)
                                    {
                                        sql_YXbsSector_in.Add(r_dt.m_id.Value);
                                    }
                                    if ((sql_YXbsSector_in.Count > Cn) || ((idx_YXbsSector + 1) == obj.XbsStationdatform.Count))
                                    {
                                        sql_YXbsSector = string.Format("(ID_XBS_STATION IN ({0}))", string.Join(",", sql_YXbsSector_in));
                                        sql_YXbsSector_in.Clear();



                                        int idx_SectorStationForMeas = 0;
                                        List<int> sql_SectorStationForMeas_in = new List<int>();
                                        string sql_SectorStationForMeas = "";
                                        ...
                                        YXbsSector Xbssect_ = new YXbsSector();
                                        Xbssect_.Format("*");
                                        Xbssect_.Filter = sql_YXbsSector; //string.Format("(ID_XBS_STATION={0})", r_dt.m_id);
                                        for (Xbssect_.OpenRs(); !Xbssect_.IsEOF(); Xbssect_.MoveNext())
                                        {
                                            SectorStationForMeas sectM = new SectorStationForMeas();
                                            ...
                                            if (sql_SectorStationForMeas_in.Count <= Cn)
                                            {
                                                sql_SectorStationForMeas_in.Add(Xbssect_.m_id.Value);
                                            }
                                            if ((sql_SectorStationForMeas_in.Count > Cn) || ((idx_SectorStationForMeas + 1) == Xbssect_.GetCount()))
                                            {
                                                sql_SectorStationForMeas = string.Format("(ID_XBS_SECTOR IN ({0}))", string.Join(",", sql_SectorStationForMeas_in));
                                                sql_SectorStationForMeas_in.Clear();




                                                int idx_YXbsSectorFreq = 0;
                                                List<int> sql_YXbsSectorFreq_in = new List<int>();
                                                string sql_YXbsSectorFreq = "";
                                                ...
                                                YXbsLinkSectorFreq linkSectorFreq = new YXbsLinkSectorFreq();
                                                linkSectorFreq.Format("*");
                                                linkSectorFreq.Filter = sql_SectorStationForMeas;//string.Format("(ID_XBS_SECTOR={0})", Xbssect_.m_id);
                                                for (linkSectorFreq.OpenRs(); !linkSectorFreq.IsEOF(); linkSectorFreq.MoveNext())
                                                {


                                                    if (sql_YXbsSectorFreq_in.Count <= Cn)
                                                    {
                                                        sql_YXbsSectorFreq_in.Add(linkSectorFreq.m_id_xbs_sectorfreq.Value);
                                                    }
                                                    if ((sql_YXbsSectorFreq_in.Count > Cn) || ((idx_YXbsSectorFreq + 1) == linkSectorFreq.GetCount()))
                                                    {
                                                        sql_YXbsSectorFreq = string.Format("(ID IN ({0}))", string.Join(",", sql_YXbsSectorFreq_in));
                                                        sql_SectorStationForMeas_in.Clear();

                                                        YXbsSectorFreq XbsFreqforsectics_ = new YXbsSectorFreq();
                                                        XbsFreqforsectics_.Format("*");
                                                        XbsFreqforsectics_.Filter = sql_YXbsSectorFreq;//string.Format("(ID={0})", linkSectorFreq.m_id_xbs_sectorfreq);
                                                        for (XbsFreqforsectics_.OpenRs(); !XbsFreqforsectics_.IsEOF(); XbsFreqforsectics_.MoveNext())
                                                        {
                                                            FrequencyForSectorFormICSM freqM = new FrequencyForSectorFormICSM();
                                                            freqM.ChannalNumber = XbsFreqforsectics_.m_channalnumber;
                                                ...
                                                string sql_YXbsSectorMaskElem = "";
                                                List<int> sql_YXbsSectorMaskElem_in = new List<int>();
                                                int idx_YXbsSectorMaskElem = 0;

                                                List<MaskElements> L_Mask = new List<MaskElements>();
                                                YXbsLinkSectorMask yXbsLinkSectorMask = new YXbsLinkSectorMask();
                                                yXbsLinkSectorMask.Format("*");
                                                yXbsLinkSectorMask.Filter = sql_SectorStationForMeas;//string.Format("(ID_XBS_SECTOR={0})", Xbssect_.m_id);
                                                for (yXbsLinkSectorMask.OpenRs(); !yXbsLinkSectorMask.IsEOF(); yXbsLinkSectorMask.MoveNext())
                                                {

                                                    if (sql_YXbsSectorMaskElem_in.Count <= Cn)
                                                    {
                                                        sql_YXbsSectorMaskElem_in.Add(yXbsLinkSectorMask.m_id_sectormaskelem.Value);
                                                    }
                                                    if ((sql_YXbsSectorMaskElem_in.Count > Cn) || ((idx_YXbsSectorMaskElem + 1) == yXbsLinkSectorMask.GetCount()))
                                                    {
                                                        sql_YXbsSectorMaskElem = string.Format("(ID IN ({0}))", string.Join(",", sql_YXbsSectorMaskElem_in));
                                                        sql_YXbsSectorMaskElem_in.Clear();


                                                        YXbsSectorMaskElem Xbsaskelements_ = new YXbsSectorMaskElem();
                                                        Xbsaskelements_.Format("*");
                                                        Xbsaskelements_.Filter = sql_YXbsSectorMaskElem;//string.Format("(ID={0})", yXbsLinkSectorMask.m_id_sectormaskelem);
                                                        for (Xbsaskelements_.OpenRs(); !Xbsaskelements_.IsEOF(); Xbsaskelements_.MoveNext())
                                                        {
                                                            MaskElements MaskElementsM = new MaskElements();
                                                            MaskElementsM.BW = Xbsaskelements_.m_bw;
