    ...
    public class ClassConvertTasks : IDisposable
    {
        ...
        public MeasTask[] ConvertTo_MEAS_TASKObjects(List<CLASS_TASKS> objs)
        {
            ...
            try
            {
                ...
                {
                    //проверка если интервал даты-времени вышел за пределы, то не выполнять загрузку
                    //KeyValuePair<YXbsMeassubtask, List<YXbsMeassubtasksta>> measStCheck = obj.meas_st.Find(z => z.Key.m_timestart <= DateTime.Now && DateTime.Now<= z.Key.m_timestop);
                    //if (measStCheck.Key == null) continue;
                    ...
                    {
                        //const int MaxExecuteParameters = 200;
                        //List<string> sqlAdditional = new List<string>();
                        //List<KeyValuePair<int?, StationDataForMeasurements>> AllStationDataForMeasurements = new List<KeyValuePair<int?, StationDataForMeasurements>>();
