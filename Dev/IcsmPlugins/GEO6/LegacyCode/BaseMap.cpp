...
#define VERIFY(a) if(a) ; else ShowMessage(String("Fail in ")+String(__FILE__)+" ("+__LINE__+")");
...
void __fastcall TBaseMapFrame::Init()
{
    ...
    // on some machines, delay is needed before starting to create custom layers
    double second = 1.0/24/60/60;
    TDateTime dt = Now();
    if (doInitDelay)
    {
        while (Now() < dt + second * initDelay)
            Application->HandleMessage();
    }
    if (doInitInfo)
        ShowMessage("Map initialized.");
}
//---------------------------------------------------------------------------
...
void __fastcall TBaseMapFrame::LoadConf(AnsiString filename)
{
    ...
    {
        //try to find the conf file at old location - in program dir
        String oldConfFileName = ChangeFileExt(Application->ExeName, ".mapconf");
        if (FileExists(oldConfFileName))
            VERIFY(CopyFile(oldConfFileName.c_str(), filename.c_str(), true));
    }
...
void __fastcall TBaseMapFrame::actConfExecute(TObject *Sender)
{
    ...
    std::auto_ptr<TdlgMapConf> dlg(new TdlgMapConf(Application, confFileName));
...
String TBaseMapFrame::GetConfFileName()
{
    ...
    CloseHandle(hToken);
    String confFileName(szHomeDirBuf);
    if (confFileName[confFileName.Length()] != '\\')
        confFileName += '\\';
    confFileName += ChangeFileExt(ExtractFileName(Application->ExeName), ".mapconf");
