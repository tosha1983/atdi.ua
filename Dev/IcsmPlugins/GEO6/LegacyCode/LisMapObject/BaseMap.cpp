...
{
    if (owner == NULL)
        throw *(new Exception(__FUNC__"(): no owner"));
...
void MapShape::ChangeDrawFont(HDC hMapDC)
{
   if(isNewFont)
      myFont = MapShape::CreateFont(hMapDC, fontName, fontSize, fontColor, fontStyle);
   else
      myFont = MapShape::CreateFont(hMapDC, "Arial", 8, clBlack, TFontStyles());;
   oldFont = ::SelectObject(hMapDC, myFont);
}

void MapShape::RestoreDrawFont(HDC hMapDC)
{
   if(oldFont != NULL)
   {
...
void MapShape::SetNewFont(AnsiString FontName, int FontSize, bool isBold, bool isItalic, bool isUnderline, TColor Color)
{
    fontName = FontName;
    fontSize = FontSize;
...
bool __fastcall MapPoint::GetExtent(DPoint& lb, DPoint& rt)
{
    lb.first = x;
    lb.second = y;
    rt.first = x;
...
void MapLink::Paint(HDC hMapDC)
{
    ...
    if(arrow == matPoint)
    {
        ...
        HGDIOBJ oldBrush = ::SelectObject(hMapDC, ::CreateSolidBrush(this->color));
...
bool __fastcall MapLink::GetExtent(DPoint& lb, DPoint& rt)
{
    lb.first = min(x1, x2);
    lb.second = min(y1, y2);
    rt.first = max(x1, x2);
...
void MapPolygon::Paint(HDC hMapDC)
{
    ...
    {
        // first point
        DPoints::iterator pi = points.begin();
        pointsInt.clear();
        ...
        if (closed)
        {
            ...
            if (filled)
            {
                HRGN rgn = ::CreatePolygonRgn(pointsInt.begin(), pointsInt.size(), ALTERNATE);
                HGDIOBJ oldBrush = ::SelectObject(hMapDC, ::CreateSolidBrush(this->color));
                ::PaintRgn(hMapDC, rgn);
                ::DeleteObject(::SelectObject(hMapDC, oldBrush));
                ::DeleteObject(rgn);
...
bool __fastcall MapPolygon::GetExtent(DPoint& lb, DPoint& rt)
{
    if (points.size() == 0)
        return false;
    else
    {
        ...
        DPoints::iterator i = points.begin();
        ...
        OLE_HANDLE hAttributeDC, LPDISPATCH RectFull, LPDISPATCH RectInvalid)
{
    ...
    for (int i = n-1; i >= 0; i--)
    {
        if (GetLayerVisible(i))
        {
            // Шрифт определяеться в "MapShape". Он полностью управляет шрифтом прорисовки текста
            ...
            try
            {
                ...
                for (sh = shapes.rbegin(); sh != shapes.rend(); sh++)
                {
                    ...
                    {
                        MapShape *t_sh = (*sh);
                        t_sh->ChangeDrawFont(hMapDC); //Меняем шрифт текста
                        try
                        {
                        	tmp_name = t_sh->name;
                        	try
                        ...
                        __finally
                        {
                            t_sh->RestoreDrawFont(hMapDC);  // Возвращаем старий шрифт текста
                        }
                    }
                }
            }
...
void __fastcall TBaseMapFrame::MapToolUsed(System::TObject * Sender, short ToolNum,
    ...
    VARIANT_BOOL Shift, VARIANT_BOOL Ctrl, VARIANT_BOOL* EnableDefault)
{
    ...
    {
        if (!distShape)
            distShape = NewShape(stLink);
        MapLink* dl = (MapLink*)distShape;
        dl->color = clRed;
        dl->width = 1;
        dl->arrow = matPoint;
        //dl->trim = 1;
        ...
        float x1, y1;
        Map->ConvertCoord(&x1, &y1, &X2, &Y2, miMapToScreen);
        WORD x = x1; WORD y = y1;

        /*
        HDC dc = ::GetDC(Map->Handle);
        dl->Paint(dc);
        ::ReleaseDC(Map->Handle, dc);

        PostMessage(Map->Handle, WM_LBUTTONDOWN,
                    //MK_LBUTTON | ((bool)Ctrl ? MK_CONTROL : 0x0) | ((bool)Shift ? MK_SHIFT : 0x0),
                    MK_LBUTTON | Ctrl & MK_CONTROL | Shift & MK_SHIFT,
                    (DWORD)x | ((DWORD)y << 16));
        */
    ...
    if (m_ToolUsed)
        m_ToolUsed(Sender, ToolNum, X1, Y1, X2, Y2, Distance, Shift, Ctrl, EnableDefault);
    if (ToolNum == miDistanceTool)
    {
        Map->Refresh();
        ShowMessage(FormatFloat("0.### км", Distance));
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TBaseMapFrame::MapMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Map->CurrentTool == miDistanceTool && Button == mbRight && distShape)
    {
        delete distShape;
    ...
    try {
        ...
        if (FileExists(filename))
        {
            ...
            if (!(bool)conf->load(TVariant(WideString(filename))))
                throw *(new Exception(__FUNC__"(): Файл не читается"));
            IXMLDOMElementPtr root = conf->documentElement;
            if (!root.IsBound())
                throw *(new Exception(__FUNC__"(): Корневой узел отсутствует"));
...
void __fastcall TBaseMapFrame::FitObjects()
{
    ...
    for (Shapes::iterator sh = shapes.begin(); sh < shapes.end(); sh++)
    {
        if ((*sh)->visible && (*sh)->layer < layers.size() && GetLayerVisible((*sh)->layer))
        {
            DPoint lb, rt;
...
void TBaseMapFrame::CheckIsShapeValid(MapShape *sh)
{
    ...
    if (posi == shapePosIdx.end())
        throw *(new Exception(__FUNC__"(): shape is not indexed"));
}
...
{
    if(idx < 0 || idx >= size)
        throw *(new Exception(String().sprintf("Неправильный индекс %s (%d из %d возможных)",
                                                msg.c_str(), idx, size)));
}
...
void __fastcall TBaseMapFrame::MapDblClick(TObject *Sender)
{
    if (Map->CurrentTool == miDistanceTool)
    {
        Map->CurrentTool == miArrowTool;
        Map->CurrentTool == miDistanceTool;
        Map->Refresh();
