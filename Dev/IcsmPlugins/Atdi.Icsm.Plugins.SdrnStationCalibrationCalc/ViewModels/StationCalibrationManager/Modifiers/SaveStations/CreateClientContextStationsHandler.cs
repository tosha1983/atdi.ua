...
namespace Atdi.Icsm.Plugins.SdrnStationCalibrationCalc.ViewModels.StationCalibrationManager.Modifiers
{
    public class CreateClientContextStationsHandler : ICommandHandler<CreateClientContextStations>
    {
        ...
        public CreateClientContextStationsHandler(AppComponentConfig config, CalcServerDataLayer dataLayer, IObjectReader objectReader, IEventBus eventBus, ILogger logger)
        {
            _config = config;
        ...
        public void Handle(CreateClientContextStations commandParameters)
        {
            ...
            try
            {
                for (int i = 0; i < clientContextStations.Length; i++)
                {
                    ...
                    if (command.ModifiedDate != null)
                    {
                        insQuery.SetValue(c => c.ModifiedDate, command.ModifiedDate);
                    }
                    insQuery.SetValue(c => c.ExternalSource, command.ExternalSource)
                    .SetValue(c => c.ExternalCode, command.ExternalCode);
                    if (command.SITE != null)
                    {
                        insQuery.SetValue(c => c.SITE.Longitude_DEC, command.SITE.Longitude_DEC)
                    ...
                    {
                        insQuery.SetValue(c => c.ANTENNA.ItuPatternCode, command.ANTENNA.ItuPatternCode)
