        ...
        public void Close()
        {
            try
            {
                if (_webSocket.State == WebSocketState.Open)
                {
                    try
                    {
                        _webSocket.CloseOutputAsync(WebSocketCloseStatus.NormalClosure, "The user initiated connection close to the sensor", CancellationToken.None).GetAwaiter().GetResult();
                    }
                    catch (Exception) { }
                }
            }
