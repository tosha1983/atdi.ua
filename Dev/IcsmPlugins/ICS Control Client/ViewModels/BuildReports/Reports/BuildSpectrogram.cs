    ...
    public class BuildSpectrogram
    {
        ...
        public void CreateBitmapSpectrogram(DataSynchronizationProcessProtocolsViewModel emit, Bitmap bm,  int ActualWidth, int ActualHeight)
        {
            ...
            using (Graphics gr = Graphics.FromImage(bm))
            {
                ...
                var stringFont = new Font("Times New Roman", 12, FontStyle.Regular);
                Height = Height - cnt;
                ...
                foreach (var lines in _option.LinesArray)
                {
                    ...
                    gr.DrawString($"{Math.Round(lines.level_dBm, 6).ToString()} дБм", stringFont, lines.LineColor, NumLine * (Width / 4) - 200, 32);
                ...
                if (m1!=null)
                {
                    ...
                    gr.DrawString($"Span: {Math.Round((stopFreq - startFreq) * 1000, 6).ToString()} КГц", stringFont, m1.LineColor, NumLine * (Width / 4) - 200, 32);
                }
                ...
                gr.DrawString(_option.XLabel, stringFont, Brushes.Black, new PointF((float)Width / 2, Height - deltaY + 32));
