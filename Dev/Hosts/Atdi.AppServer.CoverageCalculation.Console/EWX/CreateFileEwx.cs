...
{
    public static class CreateFileEwx
    {
        public static bool CreateFile(string Path, EwxData ewx, ILogger logger)
        {
            ...
            try
            {
                ...
                logger.Info(Contexts.CalcCoverages, string.Format(Events.OperationSaveEWXFileCompleted.ToString(), Path));
            }
            catch (Exception e)
            {
        ...
        private static bool CheckDiagH(ref string diagH, out string NameTag)
        {
        ...
        private static bool CheckDiagV(ref string diagV, out string NameTag)
        {
        ...
        private static string GetFormatArray(double[] inArr)
        {
            string outString = "";
            if (inArr != null)
            {
        ...
        private static double[] ParseStringToArray(string diag)
        {
        ...
        private static double[] InterpolationForICSTelecomHorizontal(double[] inArray)
        {
            ...
            if (inArray != null)
            {
                ...
                {
                    throw new Exception($"Incorrect count element in inArray ({inArray.Length} < {MinCount})");
                }
                for (int i = 0; i < 36; i++)
                {
            ...
            {
                throw new Exception("inArray is NULL!");
            }
            return result;
        }


        private static double[] InterpolationForICSTelecomVertical(double[] inArray)
        {
