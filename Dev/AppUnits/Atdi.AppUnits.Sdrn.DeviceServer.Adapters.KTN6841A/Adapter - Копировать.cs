...
namespace Atdi.AppUnits.Sdrn.DeviceServer.Adapters.KTN6841A
{
    public class Adapter : IAdapter
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        int st = -1;
                        double freqstep = 0;
                        //double newFreqCentr = 0;
                        double freqstart = 0;
                        //////FftPoints = (uint)FftPointsIndex;
                        //////fSpan = gSpans[needSpanIndex];
                        //////int steps = (int)needsteps;
                        FftPoints = numFftPoints;
                        fSpan = span;
                        ...
                        for (ulong t = 0; t < traceCountToMeas; t++)
                        {
                            ...
                            for (int s = 0; s < steps; s++)//идем по частотам
                            {
                                ...
                                if (GetTraceData(ref LevelArrTemp2, ref points, ref overloadonsubstep, ref freqstart, ref freqstep))
                                {
                                    if (!freqResSet && s == 0 )
                                    {
                                        ...
                                        for (int rf = 0; rf < steps * points; rf++)
                                        {
                                            double ddd = resFreqStart + rf * resFreqStep;
                                            if (ddd > stopfreq)
                                            {
                                                resFreqStopIndex = rf;
                                            }
                            ...
                            SetTraceData(LevelArrTemp, (uint)(points * steps));
        ...
        private bool GetTraceData(ref float[] trace, ref uint points, ref bool overload, ref double freqstart, ref double freqstep)
        {
            bool res = false;
