...
namespace Atdi.AppUnits.Sdrn.CalcServer.Tasks.Iterations
{
    ...
    /// </summary>
    public class Ge06CalcFieldStrengthInPoint : IIterationHandler<BroadcastingFieldStrengthInPointGE06, PointResult>
    {
        ...
        public Ge06CalcFieldStrengthInPoint(
        ...
        public PointResult Run(ITaskContext taskContext, BroadcastingFieldStrengthInPointGE06 data)
        {
            var profileOptions = GetPropagationModel(data);
        ...
        private ProfileOptions GetPropagationModel(BroadcastingFieldStrengthInPointGE06 data)
        {
            var options = ProfileOptions.Empty;

            if (((CalculationType)data.Ge06TaskParameters.CalculationTypeCode == CalculationType.CreateContoursByDistance)
                || ((CalculationType)data.Ge06TaskParameters.CalculationTypeCode == CalculationType.CreateContoursByFS))
            {
                options = DefineProfileOptions(data.PropagationModel);
            }
            else if (((CalculationType)data.Ge06TaskParameters.CalculationTypeCode == CalculationType.ConformityCheck)
               || ((CalculationType)data.Ge06TaskParameters.CalculationTypeCode == CalculationType.FindAffectedADM))
            {
