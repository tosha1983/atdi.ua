...
{
    public class CommandsSendEventPipelineHandler : IPipelineHandler<ClientMeasTaskPipebox, ClientMeasTaskPiperesult>
    {
        ...
        public CommandsSendEventPipelineHandler(IEventEmitter eventEmitter, IDataLayer<EntityDataOrm> dataLayer, ISdrnServerEnvironment environment, ISdrnMessagePublisher messagePublisher, ILogger logger)
        {
        ...
        ///  если нет сведений о AggregationServer, то делается вывод о том, что нужно просто отправить уведомления в обработчики (OnReceivedDelMeasTaskEvent,OnReceivedNewMeasTaskEvent,OnReceivedStopMeasTaskEvent)
        ...
        {
            using (this._logger.StartTrace(Contexts.ThisComponent, Categories.CommandsSendEventPipelineHandler, this))
            {
