    ...
    public static class CalculationAntennaPattern
    {
        public static DiagrammPoint[] Calc(string points, AntennaPatternType antennaPatternType, double gain)
        {
            ...
            if (patternType == TypePattern.WIEN)
            {
                ...
                if ((antennaPatternType == AntennaPatternType.HH) || (antennaPatternType == AntennaPatternType.HV))
                {
                    for (int i = 0; i < AnglesH.Length; i++)
                    {
                        ...
                        {
                            Angle = AnglesH[i],
                            Loss = (float)(gain - diag.GetLossesAmount(AnglesH[i]))
                        });
                    }
                }
                else if ((antennaPatternType == AntennaPatternType.VH) || (antennaPatternType == AntennaPatternType.VV))
                {
                    for (int i = 0; i < AnglesV.Length; i++)
                    {
                        ...
                        {
                            Angle = AnglesV[i],
                            Loss = (float)(gain - diag.GetLossesAmount(AnglesV[i]))
                        });
                    }
                }
