...
{
    public class CheckErrorAdapterForMeasResultTaskWorker
    {
        ...
        public CheckErrorAdapterForMeasResultTaskWorker(AppServerComponentConfig config, IController controller, ILogger logger)
        {
            this._config = config;
        ...
        public DeviceServerCancellationData Handle(ITaskContext<ClientReadyTakeMeasResultTask, OnlineMeasurementProcess> context, ExceptionProcessLevel exceptionProcessLevel, out DeviceServerResultLevel deviceServerResultLevel)
        {
                ...
                switch (exceptionProcessLevel._failureReason)
                {
                    ...
                    case CommandFailureReason.DeviceIsBusy:
                        for (int i=0; i<50; i++)
                        {
                            ...
                            if (sendCommandForMeasResultTaskWorker.Handle(context, deviceCommand, out deviceServerResultLevel) == false)
                            {
