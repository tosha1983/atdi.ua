...
namespace Atdi.AppUnits.Sdrn.DeepServices.EarthGeometry
{
    public class EarthGeometricService : IEarthGeometricService
    {
        ...
        private PointEarthGeometricWithAzimuth[] CheckInterSectPoint(PointEarthGeometric pointEarthGeometricBaryCentr, PointEarthGeometric[] contourPoints, double distance, double azimuth)
        {
            ...
            var coordRecalcNew = CalculationCoordinateByLengthAndAzimuth(pointEarthGeometricBaryCentr.Longitude, pointEarthGeometricBaryCentr.Latitude, MaxDistance, azimuth, false);
            ...
            for (int j = 0; j < contourPoints.Length - 1; j++)
            {
                ...
                {
                    var coordRecalc = new PointEarthGeometric();
                    int cntIter = 0;
                    for (int az = 0; az < 360; az++)
                    {
                        coordRecalc = CalculationCoordinateByLengthAndAzimuth(poi_.Longitude, poi_.Latitude, distance, az);
                        if (CheckHitting(contourPoints, coordRecalc) == false)
                        {
                            cntIter++;
                        }
                        if (cntIter > 20)
            ...
            if (segments_intersect)
            {
                ...
                for (int az = 0; az < 360; az++)
                {
                    coordRecalc = CalculationCoordinateByLengthAndAzimuth(poi.Longitude, poi.Latitude, distance, az);
                    if (CheckHitting(contourPoints, coordRecalc) == false)
                    {
                        cntIter++;
                    }
            ...
            out PointEarthGeometric close_p1, out PointEarthGeometric close_p2)
        {
            ...
            double t1 = ((p1.Longitude - p3.Longitude) * dy34 + (p3.Latitude - p1.Latitude) * dx34)  / denominator;
            if (double.IsInfinity(t1))
            {
            
                lines_intersect = false;
                segments_intersect = false;
                intersection = new PointEarthGeometric(double.NaN, double.NaN, CoordinateUnits.deg);
                close_p1 = new PointEarthGeometric(double.NaN, double.NaN, CoordinateUnits.deg);
                close_p2 = new PointEarthGeometric(double.NaN, double.NaN, CoordinateUnits.deg);
                return;
            }
            lines_intersect = true;

            double t2 =  ((p3.Longitude - p1.Longitude) * dy12 + (p1.Latitude - p3.Latitude) * dx12)/ -denominator;
            
            intersection = new PointEarthGeometric(p1.Longitude + dx12 * t1, p1.Latitude + dy12 * t1, CoordinateUnits.deg);
