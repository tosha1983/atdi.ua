...
{
    public class MeasTasksMasterServerSendEventPipelineHandler : IPipelineHandler<ClientMeasTaskPipebox, ClientMeasTaskPiperesult>
    {
        ...
        public MeasTasksMasterServerSendEventPipelineHandler(IDataLayer<EntityDataOrm> dataLayer, IEventEmitter eventEmitter, ISdrnServerEnvironment environment, IPublisher publisher, ILogger logger)
        {
        ...
        /// <summary>
        /// Конвеерный обработчик задач, который выполняет "группировку" подзадач по списку AggregationServer'ов
        ...
        {
            using (this._logger.StartTrace(Contexts.ThisComponent, Categories.MeasTasksMasterServerSendEventPipelineHandler, this))
            {
                ...
                if ((data.MeasTasksWithAggregationServerPipeBox != null) && (data.MeasTasksWithAggregationServerPipeBox.Length > 0))
                {
                    ...
                    {
                        var retEnvelope = this._publisher.CreateEnvelope<MeasTaskToAggregationServer, CreateMeasTaskPipebox>();
                        retEnvelope.To = data.AggregationServerInstancesPipeBox[i];
                        retEnvelope.DeliveryObject = new CreateMeasTaskPipebox() { MeasTaskPipeBox = data.MeasTasksWithAggregationServerPipeBox[i], MeasTaskModePipeBox = data.MeasTaskModePipeBox };
