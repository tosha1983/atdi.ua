...
namespace Atdi.AppUnits.Sdrn.DeviceServer.Adapters.SignalHound
{
    internal static class DeviceReset
    {
        ...
        private static string GetDeviceInstance(SafeDeviceInformationSetHandle infoSet, SP_DEVINFO_DATA devInfo)
        {
            Int32 requiredSize = 2048;
            StringBuilder sb = new StringBuilder(requiredSize);
            if (!SetupDiGetDeviceInstanceId(infoSet, ref devInfo, sb, 0, out requiredSize))
            {
                switch (Marshal.GetLastWin32Error())
                {
                    case ERROR_INSUFFICIENT_BUFFER:
        ...
        public static bool SignalHoundBB60StatePrepared(string SerialNumber, bool enable)
        {
            ...
            if (SerialNumber != "")
            {
                string BB60CWithSN = @"USB\VID_2817&PID_0002\" + SerialNumber;// @"USB\VID_2817&PID_0002";
                string BB60C_2WithSN = @"USB\VID_2817&PID_0005\" + SerialNumber;//@"USB\VID_2817&PID_0005";
                string DeviceName = "BB60";
                //по идее гуид класса устройств с USB, по одному нему нельзя т.к. попытается грохнуть все с USB
                Guid USBGuid = new Guid("36fc9e60-c465-11cf-8056-444553540000");
            ...
            {
                throw new Exception("Serial number must be installed");
            }
