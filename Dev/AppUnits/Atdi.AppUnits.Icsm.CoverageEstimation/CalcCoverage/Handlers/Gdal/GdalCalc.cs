...
namespace Atdi.AppUnits.Icsm.CoverageEstimation.Handlers
{
    ...
    public class GdalCalc
    {
        ...
        private TFWParameter GetTFWParameter(string TiffFile)
        {
        ...
        private Coordinate GetCoordinateAfterRecalc(int MatrixY, int MatrixX, int MatrixHeight, int MatrixWidth, TFWParameter tFWParameterBlank, TFWParameter tFWParameterStation)
        {
        ...
        public void CheckOutTIFFFilesDirectorys(DataConfig dataConfig)
        {
            for (int i = 0; i < dataConfig.CodeOperatorAndStatusesConfig.Length; i++)
            {
                ...
                if (codeOperator != null)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(codeOperator.StandardConfig.OutTIFFFilesDirectory))
                        {
                            if (!Directory.Exists(codeOperator.StandardConfig.OutTIFFFilesDirectory))
                            {
                                Directory.CreateDirectory(codeOperator.StandardConfig.OutTIFFFilesDirectory);
                            }
                        }
