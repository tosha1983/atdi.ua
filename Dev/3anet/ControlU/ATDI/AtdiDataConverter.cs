        ...
        public LocalMeasSdrTask_v2 ConvertToLocal(MeasTask task)
        {
            ...
            if (task.Stations != null && task.Stations.Length > 0)
            {
                ...
                foreach (string ss in tech)
                {
                    ...
                    {
                        task_table_name = "atdi_task_v2_" + task.TaskId + "_" + ss,
                        result_table_name = "atdi_task_result_v2_" + task.TaskId + "_" + ss,
                        tech = ss,
                ...
                {
                    task_table_name = "atdi_task_v2_" + task.TaskId + "_unknown",
                    result_table_name = "atdi_task_result_v2_" + task.TaskId + "_unknown",
                    tech = "unknown",
                ...
                to.routes_tb_name = "atdi_task_v2_" + task.TaskId + "_routes";
                ...
                for (int i = 0; i < to.data_from_tech.Count(); i++)
                {
                    for (int j = 0; j < task.Stations.Length; j++)
                    {
                        if (task.Stations[j].Standard.ToUpper().Contains(to.data_from_tech[i].tech))
                        {
                            ...
                            lic.icsm_id = (int)task.Stations[j].License.IcsmId;//проверку
                            st.license = lic;
                            ...
                            si.region = task.Stations[j].Site.Region;//проверку
                            ...
                            for (int s = 0; s < task.Stations[j].Sectors.Length; s++)
                            {
                                ...
                                for (int f = 0; f < task.Stations[j].Sectors[s].Frequencies.Length; f++)
                                {
                                    ...
                                    freq.id = (int)task.Stations[j].Sectors[s].Frequencies[f].Id;//проверку
