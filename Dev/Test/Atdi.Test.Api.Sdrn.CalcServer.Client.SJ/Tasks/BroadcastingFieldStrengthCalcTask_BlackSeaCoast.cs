...
{
    public static class BroadcastFieldStrengthCalcTaskBlackSeaCoast
    {
        ...
        ///// 41-42 mix short distance
        //private static double[] sitesLatitudes = new double[] { 46.043564 };
        //private static double[] sitesLongitudes = new double[] { 30.397428 };
        //private static double[] sitesHeights = new double[] { 10 };

        //private static double[] pointsLatitudes = new double[] { 46.040558 };
        //private static double[] pointsLongitudes = new double[] { 30.401205 };
        //private static double[] pointsHeights = new double[] { 10 };

        ///// 43-44 mix low rx ant


        //private static double[] sitesLatitudes = new double[] { 46.067596 };
        //private static double[] sitesLongitudes = new double[] { 30.435252 };
        //private static double[] sitesHeights = new double[] { 10 };

        //private static double[] pointsLatitudes = new double[] { 46.024409 };
        //private static double[] pointsLongitudes = new double[] { 30.466242 };
        //private static double[] pointsHeights = new double[] { 2 };

        //// 45-46 mix low rx ant
        //private static double[] sitesLatitudes = new double[] { 45.782642 };
        //private static double[] sitesLongitudes = new double[] { 31.363665 };
        //private static double[] sitesHeights = new double[] { 10 };

        //private static double[] pointsLatitudes = new double[] { 45.740109 };
        //private static double[] pointsLongitudes = new double[] { 31.411184 };
        //private static double[] pointsHeights = new double[] { 2 };
        ...
        private static long CreateCalcTask(WebApiDataLayer dataLayer, IQueryExecutor executor, long contextId, long stationId, PointOnMap pointOnMap)
        {
            ...
            var insQuery = dataLayer.GetBuilder<ICalcTask>()
                ...
                .SetValue(c => c.TypeCode, (byte)CalcTaskType.Gn06CalcTask)
                .SetValue(c => c.TypeName, CalcTaskType.Gn06CalcTask.ToString())
                ;
            var taskPk = executor.Execute<ICalcTask_PK>(insQuery);

            var argsQuery = dataLayer.GetBuilder<IGn06Args>()
                    .Create()
        ...
        private static long CreatePlannedCalcTask(WebApiDataLayer dataLayer, IQueryExecutor executor, long contextId, long assignmentId, PointOnMap pointOnMap)
        {
            //var rand = new Random();
            var insQuery = dataLayer.GetBuilder<IContextPlannedCalcTask>()
                    ...
                    .SetValue(c => c.TypeCode, (byte)CalcTaskType.Gn06CalcTask)
                    .SetValue(c => c.TypeName, CalcTaskType.Gn06CalcTask.ToString())
                    .SetValue(c => c.StartNumber, (int)assignmentId)
                ;
            var plannedTaskPk = executor.Execute<IContextPlannedCalcTask_PK>(insQuery);

            var defaultArgsQuery = dataLayer.GetBuilder<IGn06ArgsDefault>()
                    .Create()
                    .SetValue(c => c.TaskId, plannedTaskPk.Id)
                    //.SetValue(c => c.STATION.Id, assignmentId)
                    //.SetValue(c => c.PointAltitude_m, 10)
                    //.SetValue(c => c.PointLongitude_DEC, 24.03175 + 0.1 * (0.5 - rand.NextDouble()))
                    //.SetValue(c => c.PointLatitude_DEC, 49.86411 + 0.1 * (0.5 - rand.NextDouble()))
                    //.SetValue(c => c.PointAltitude_m, pointOnMap.Hei_m)
        ...
        private static IGn06Result WaitForCalcResult(WebApiDataLayer dataLayer, IQueryExecutor executor, long calcTaskId, long calcResultId)
        {
            var cancel = false;
            var resultObject = dataLayer.ProxyInstanceFactory.Create<IGn06Result>();
            ...
            while (!cancel)
            {
                ...
                cancel = executor.ExecuteAndFetch(checkQuery, reader =>
                {
                    ...
                    && status != CalcResultStatusCode.Processing)
                    {
                        ...
                        //var resultQuery = dataLayer.GetBuilder<IGn06Result>()
                        //    .Read()
                        //    .Select(c => c.FS_dBuVm)
