...
{
	public static class RefSpectrumByDriveTestsCalcTask
    {
		...
		private static long DefineClientContext(WebApiDataLayer dataLayer, IQueryExecutor executor, long projectId, Guid ownerContextId)
		{
			...
			if (contextId == 0)
			{
				...
				var stations = new long[10];
				for (var i = 0; i < stations.Length; i++)
				{
					stations[i] = CreateClientContextStation(dataLayer, executor, contextId);
					// планируемые задачи для каждой станции
					CreatePlannedCalcTask(dataLayer, executor, contextId, stations[i]);
				}
		...
		private static long CreateClientContextStation(WebApiDataLayer dataLayer, IQueryExecutor executor, long contextId)
		{
			var rand = new Random();
		...
		private static long CreateCalcTask(WebApiDataLayer dataLayer, IQueryExecutor executor, long contextId)
		{
			var rand = new Random();
			var insQuery = dataLayer.GetBuilder<ICalcTask>()
				...
				.SetValue(c => c.CreatedDate, DateTimeOffset.Now)
				.SetValue(c => c.CONTEXT.Id, contextId)
				.SetValue(c => c.MapName, "Kyiv5")
				.SetValue(c => c.OwnerInstance, "ICSMP-CBD12-A00-3579")
				...
				.SetValue(c => c.TypeCode, (byte)CalcTaskType.RefSpectrumByDriveTestsCalcTask)
				.SetValue(c => c.TypeName, CalcTaskType.RefSpectrumByDriveTestsCalcTask.ToString())
				;
			var taskPk = executor.Execute<ICalcTask_PK>(insQuery);

			var argsQuery = dataLayer.GetBuilder<IRefSpectrumByDriveTestsArgs>()
					.Create()
			...
			Console.WriteLine($"The new Ref Spectrum Calc Task was created with ID #{taskPk.Id} ");

			return taskPk.Id;
		}

		private static long CreatePlannedCalcTask(WebApiDataLayer dataLayer, IQueryExecutor executor, long contextId, long stationId)
		{
			var rand = new Random();
		...
		private static void MakeCalcTaskAvailable(WebApiDataLayer dataLayer, IQueryExecutor executor, long taskId, long contextId)
		{
			var updQuery = dataLayer.GetBuilder<ICalcTask>()
				...
				.Filter(c => c.OwnerInstance, "ICSMP-CBD12-A00-3579")
				.Filter(c => c.CONTEXT.Id, contextId)
				// фильтр по ключу
		...
		private static long CreateCalcTaskResult(WebApiDataLayer dataLayer, IQueryExecutor executor, long taskId)
		{
			var insQuery = dataLayer.GetBuilder<ICalcResult>()
				...
				.SetValue(c => c.CallerInstance, "ICSMP-CBD12-A00-3579")
		...
		private static void RunCalcTask(WebApiDataLayer dataLayer, IQueryExecutor executor, long taskId, long resultId)
		{
			var updQuery = dataLayer.GetBuilder<ICalcResult>()
				...
				// необязательно, но можем усиливать фильтрацию текущим контекстом
				.Filter(c => c.CallerInstance, "ICSMP-CBD12-A00-3579")
