...
namespace ControlU.ATDI
{
    public class AtdiDataConverter
    {
                ...
                for (int rt = 0; rt < task.data_from_tech[t].ResultItems.Count(); rt++)
                {
                    ...
                    for (int lm = 0; lm < task.data_from_tech[t].ResultItems[rt].level_results.Count(); lm++)
                    {
                        ...
                        if (task.data_from_tech[t].ResultItems[rt].level_results[lm].difference_time_stamp_ns > -1)
                            lmr.DifferenceTimeStamp_ns = (double)task.data_from_tech[t].ResultItems[rt].level_results[lm].difference_time_stamp_ns;
                        if (task.data_from_tech[t].ResultItems[rt].level_results[lm].level_dbm > -1000)
                            lmr.Level_dBm = task.data_from_tech[t].ResultItems[rt].level_results[lm].level_dbm;
                        if (task.data_from_tech[t].ResultItems[rt].level_results[lm].level_dbmkvm > -1000)
                            lmr.Level_dBmkVm = task.data_from_tech[t].ResultItems[rt].level_results[lm].level_dbmkvm;

                        lmr.Location = new Atdi.DataModels.Sdrns.GeoLocation() { };
                        if (task.data_from_tech[t].ResultItems[rt].level_results[lm].location.agl > -100000)
                            lmr.Location.AGL = task.data_from_tech[t].ResultItems[rt].level_results[lm].location.agl;
                        if (task.data_from_tech[t].ResultItems[rt].level_results[lm].location.asl > -100000)
                            lmr.Location.ASL = task.data_from_tech[t].ResultItems[rt].level_results[lm].location.asl;
                        lmr.Location.Lat = task.data_from_tech[t].ResultItems[rt].level_results[lm].location.latitude;
                        lmr.Location.Lon = task.data_from_tech[t].ResultItems[rt].level_results[lm].location.longitude;
        ...
        public Atdi.DataModels.Sdrns.Device.MeasResults ConvertToATDI(DB.Track track)
        {
            Atdi.DataModels.Sdrns.Device.MeasResults to = new Atdi.DataModels.Sdrns.Device.MeasResults() { };
            to.ResultId = MainWindow.db_v2.ATDI_AddResultID().ToString();//"";//попровить на id
            to.Status = "";
            ...
            for (int t = 0; t < track.Data.Count(); t++) //2; t++)//  //task.data_from_tech.Count(); t++)
            {
                ...
                //smr.SectorId = task.data_from_tech[t].ResultItems[rt].id_sector;
                //smr.TaskGlobalSid = task.data_from_tech[t].ResultItems[rt].station_identifier_atdi;
                smr.RealGlobalSid = track.Data[t].gcid;//task.data_from_tech[t].ResultItems[rt].station_identifier_from_radio;
                smr.Standard = track.Data[t].tech;
                //smr.Status = task.data_from_tech[t].ResultItems[rt].status;
                smr.GeneralResult = new Atdi.DataModels.Sdrns.Device.GeneralMeasResult() { };
                ...
                //}
                #endregion
                smr.GeneralResult.CentralFrequency_MHz = (double)(track.Data[t].freq / 1000000);
                ...
                //if (task.data_from_tech[t].ResultItems[rt].station_sys_info.type_cdmaevdo != "")
                //    ssi.TypeCDMAEVDO = task.data_from_tech[t].ResultItems[rt].station_sys_info.type_cdmaevdo;
                if (track.Data[t].information_blocks.Length > 0)
                {
                    List<StationSysInfoBlock> ssibs = new List<StationSysInfoBlock>() { };
                    for (int bl = 0; bl < track.Data[t].information_blocks.Length; bl++)
                    {
                        StationSysInfoBlock ssib = new StationSysInfoBlock() { };
                        ssib.Type = track.Data[t].information_blocks[bl].type;
                        ssib.Data = track.Data[t].information_blocks[bl].datastring;
                        ssibs.Add(ssib);
                    }
                    ssi.InfoBlocks = ssibs.ToArray();
                }
                ...
                #endregion ssi
                smr.GeneralResult.StationSysInfo = ssi;
                smr.GeneralResult.MeasStartTime = DateTime.MaxValue;
                smr.GeneralResult.MeasFinishTime = DateTime.MinValue;
                List<LevelMeasResult> lmrs = new List<LevelMeasResult>() { };
                for (int lm = 0; lm < track.Data[t].level_results.Count(); lm++)
                {
                    LevelMeasResult lmr = new LevelMeasResult() { };
                    lmr.MeasurementTime = track.Data[t].level_results[lm].measurement_time;
                    if (lmr.MeasurementTime < smr.GeneralResult.MeasStartTime)
                    {
                        smr.GeneralResult.MeasStartTime = lmr.MeasurementTime;
                    }
                    if (lmr.MeasurementTime > smr.GeneralResult.MeasStartTime)
                    {
                        smr.GeneralResult.MeasFinishTime = lmr.MeasurementTime;
                    }
                    
                    if (track.Data[t].level_results[lm].difference_time_stamp_ns > -1)
                        lmr.DifferenceTimeStamp_ns = (double)track.Data[t].level_results[lm].difference_time_stamp_ns;
                    if (track.Data[t].level_results[lm].level_dbm > -1000)
                        lmr.Level_dBm = track.Data[t].level_results[lm].level_dbm;
                    if (track.Data[t].level_results[lm].level_dbmkvm > -1000)
                        lmr.Level_dBmkVm = track.Data[t].level_results[lm].level_dbmkvm;

                    lmr.Location = new Atdi.DataModels.Sdrns.GeoLocation() { };
                    if (track.Data[t].level_results[lm].location.agl > -100000)
                        lmr.Location.AGL = track.Data[t].level_results[lm].location.agl;
                    if (track.Data[t].level_results[lm].location.asl > -100000)
                        lmr.Location.ASL = track.Data[t].level_results[lm].location.asl;
                    lmr.Location.Lat = track.Data[t].level_results[lm].location.latitude;
                    lmr.Location.Lon = track.Data[t].level_results[lm].location.longitude;
