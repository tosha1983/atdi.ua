...
namespace ControlU.Equipment
{
    public class TSMxReceiver : INotifyPropertyChanged
    {
        ...
        private void TechIsEnabled_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                MainWindow.db_v2.Atdi_LevelResults_DistanceStep = (decimal)App.Sett.MeasMons_Settings.TrackDistanceStep;
                MainWindow.db_v2.Atdi_LevelsMeasurementsCar_TimeStep = new TimeSpan(0, 0, App.Sett.MeasMons_Settings.TrackTimeStep);
                ...
                if (GSMIsRuning == true && IdentificationData.GSM.BTS.Count() > 0)
                {
                    ...
                    for (int i = 0; i < IdentificationData.GSM.BTS.Count; i++)
                    {
                        if (IdentificationData.GSM.BTS[i].FullData)
                        {
                            ...
                            #endregion
                            #region в измерения
                            if (IsMeasMon && IdentificationData.GSM.BTS[i].ATDI_Identifier_Find != 0 && IdentificationData.GSM.BTS[i].ATDI_FreqCheck_Find != 0)
                            {
                                    ...
                                    new TimeSpan(MainWindow.gps.LocalTime.Ticks - IdentificationData.GSM.BTS[i].LastDetectionLevelUpdete) < new TimeSpan(0, 0, 0, 0, TimeAddAfterLevelUpdate))
                                {
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == true)
                                    {
                                        ...
                                        foreach (DB.localatdi_task_with_tech restech in MainWindow.db_v2.AtdiTask.data_from_tech)
                                        {
                                            ...
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    //DB.localatdi_result_item resi = restech.ResultItems[k];                                                
                                                    if (restech.ResultItems[k].id_station == IdentificationData.GSM.BTS[i].ATDI_id_station &&
                                                    restech.ResultItems[k].id_sector == IdentificationData.GSM.BTS[i].ATDI_id_sector &&
                                                    restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.GSM.BTS[i].FreqDn &&
                                                    restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.GSM.BTS[i].BSIC &&
                                                    restech.ResultItems[k].station_identifier_from_radio == IdentificationData.GSM.BTS[i].GCID)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                                                            FindInTaskResults = true;
                                                            FindInResulData = true;
                                                            sd = restech.ResultItems[k].spec_data;
                                                            bd = restech.ResultItems[k].bw_data;
                                                            cd = restech.ResultItems[k].cp_data;
                                                            station_sys_info = restech.ResultItems[k].station_sys_info;
                                                            _ChannelStrenght = restech.ResultItems[k].meas_strength;
                                                            _MeasCorrectness = restech.ResultItems[k].meas_correctness;
                                                            if (restech.ResultItems[k].freq_centr_perm != 0 && bd.NdBResult[1] > -1 && bd.NdBResult[2] > -1)
                                                            { _DeltaFreqMeasured = Math.Round(((Math.Abs(((sd.Trace[bd.NdBResult[1]].freq + sd.Trace[bd.NdBResult[2]].freq) / 2) - (restech.ResultItems[k].freq_centr_perm))) / (restech.ResultItems[k].freq_centr_perm / 1000000)), 3); }
                                                            else if (restech.ResultItems[k].freq_centr_perm == 0 && bd.NdBResult[1] > -1 && bd.NdBResult[2] > -1)
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == false)
                                    {
                                        ...
                                        foreach (DB.localatdi_unknown_result_with_tech restech in MainWindow.db_v2.AtdiUnknownResult.data_from_tech)
                                        {
                                            if (restech.tech.Contains("GSM"))
                                            {
                                                ...
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.GSM.BTS[i].FreqDn &&
                                                    restech.ResultItems[k].station_identifier_from_radio_s0 == IdentificationData.GSM.BTS[i].MCC &&
                                                    restech.ResultItems[k].station_identifier_from_radio_s1 == IdentificationData.GSM.BTS[i].MNC &&
                                                    restech.ResultItems[k].station_identifier_from_radio_s2 == IdentificationData.GSM.BTS[i].LAC &&
                                                    restech.ResultItems[k].station_identifier_from_radio_s3 == IdentificationData.GSM.BTS[i].CID &&
                                                    restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.GSM.BTS[i].BSIC)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                ...
                if (UMTSIsRuning == true && IdentificationData.UMTS.BTS.Count() > 0)
                {
                    ...
                    for (int i = 0; i < IdentificationData.UMTS.BTS.Count; i++)
                    {
                        if (IdentificationData.UMTS.BTS[i].FullData)
                        {
                            ...
                            #endregion
                            #region в измерения
                            if (IsMeasMon && IdentificationData.UMTS.BTS[i].ATDI_Identifier_Find != 0 && IdentificationData.UMTS.BTS[i].ATDI_FreqCheck_Find != 0)
                            {
                                #region найдено ли в измерениях уже
                                bool FindInMeasData = false;
                                if (MainWindow.db_v2.MeasMon.Data.Count > 0)
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == true)
                                    {
                                        ...
                                        foreach (DB.localatdi_task_with_tech restech in MainWindow.db_v2.AtdiTask.data_from_tech)
                                        {
                                            if (restech.tech.Contains("UMTS") || restech.tech.ToLower().Contains("unknown"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].id_station == IdentificationData.UMTS.BTS[i].ATDI_id_station &&
                                                    restech.ResultItems[k].id_sector == IdentificationData.UMTS.BTS[i].ATDI_id_sector &&
                                                    restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.UMTS.BTS[i].FreqDn &&
                                                    restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.UMTS.BTS[i].SC &&
                                                    restech.ResultItems[k].station_identifier_from_radio == IdentificationData.UMTS.BTS[i].GCID)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                                                            FindInTaskResults = true;
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == false)
                                    {
                                        ...
                                        foreach (DB.localatdi_unknown_result_with_tech restech in MainWindow.db_v2.AtdiUnknownResult.data_from_tech)
                                        {
                                            if (restech.tech.Contains("UMTS"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.UMTS.BTS[i].FreqDn &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s0 == IdentificationData.UMTS.BTS[i].MCC &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s1 == IdentificationData.UMTS.BTS[i].MNC &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s2 == IdentificationData.UMTS.BTS[i].LAC &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s3 == IdentificationData.UMTS.BTS[i].CID &&
                                                        restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.UMTS.BTS[i].SC)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                ...
                if (LTEIsRuning == true && IdentificationData.LTE.BTS.Count() > 0)
                {
                    ...
                    for (int i = 0; i < IdentificationData.LTE.BTS.Count; i++)
                    {
                        if (IdentificationData.LTE.BTS[i].FullData)
                        {
                            ...
                            #endregion
                            #region в измерения
                            if (IsMeasMon && IdentificationData.LTE.BTS[i].ATDI_Identifier_Find != 0 && IdentificationData.LTE.BTS[i].ATDI_FreqCheck_Find != 0)
                            {
                                #region найдено ли в измерениях уже
                                bool FindInMeasData = false;
                                if (MainWindow.db_v2.MeasMon.Data.Count > 0)
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == true)
                                    {
                                        ...
                                        foreach (DB.localatdi_task_with_tech restech in MainWindow.db_v2.AtdiTask.data_from_tech)
                                        {
                                            if (restech.tech.Contains("LTE") || restech.tech.ToLower().Contains("unknown"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].id_station == IdentificationData.LTE.BTS[i].ATDI_id_station &&
                                                    restech.ResultItems[k].id_sector == IdentificationData.LTE.BTS[i].ATDI_id_sector &&
                                                    restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.LTE.BTS[i].FreqDn &&
                                                    restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.LTE.BTS[i].PCI &&
                                                    restech.ResultItems[k].station_identifier_from_radio == IdentificationData.LTE.BTS[i].GCID)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                                                            FindInTaskResults = true;
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == false)
                                    {
                                        ...
                                        foreach (DB.localatdi_unknown_result_with_tech restech in MainWindow.db_v2.AtdiUnknownResult.data_from_tech)
                                        {
                                            if (restech.tech.Contains("LTE"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.LTE.BTS[i].FreqDn &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s0 == IdentificationData.LTE.BTS[i].MCC &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s1 == IdentificationData.LTE.BTS[i].MNC &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s2 == IdentificationData.LTE.BTS[i].eNodeBId &&
                                                        restech.ResultItems[k].station_identifier_from_radio_s3 == IdentificationData.LTE.BTS[i].CID &&
                                                        restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.LTE.BTS[i].PCI)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                ...
                if (CDMAIsRuning == true && IdentificationData.CDMA.BTS.Count() > 0)
                {
                    ...
                    for (int i = 0; i < IdentificationData.CDMA.BTS.Count; i++)
                    {
                        if (IdentificationData.CDMA.BTS[i].FullData)
                        {
                            ...
                            #endregion
                            #region в измерения
                            if (IsMeasMon && IdentificationData.CDMA.BTS[i].ATDI_Identifier_Find != 0 && IdentificationData.CDMA.BTS[i].ATDI_FreqCheck_Find != 0)
                            {
                                #region найдено ли в измерениях уже
                                bool FindInMeasData = false;
                                if (MainWindow.db_v2.MeasMon.Data.Count > 0)
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == true)
                                    {
                                        ...
                                        foreach (DB.localatdi_task_with_tech restech in MainWindow.db_v2.AtdiTask.data_from_tech)
                                        {
                                            if (restech.tech.Contains("CDMA") || restech.tech.ToLower().Contains("unknown"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].id_station == IdentificationData.CDMA.BTS[i].ATDI_id_station &&
                                                    restech.ResultItems[k].id_sector == IdentificationData.CDMA.BTS[i].ATDI_id_sector &&
                                                    restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.CDMA.BTS[i].FreqDn &&
                                                    restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.CDMA.BTS[i].PN &&
                                                    restech.ResultItems[k].station_identifier_from_radio == IdentificationData.CDMA.BTS[i].GCID)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
                                                        {
                                                            FindInTaskResults = true;
                                    ...
                                    if (MainWindow.db_v2.MeasMon.FromTask == false)
                                    {
                                        ...
                                        foreach (DB.localatdi_unknown_result_with_tech restech in MainWindow.db_v2.AtdiUnknownResult.data_from_tech)
                                        {
                                            if (restech.tech.Contains("CDMA"))
                                            {
                                                for (int k = 0; k < restech.ResultItems.Count; k++)
                                                {
                                                    if (restech.ResultItems[k].spec_data.FreqCentr == IdentificationData.CDMA.BTS[i].FreqDn &&
                                                        restech.ResultItems[k].station_identifier_from_radio_tech_sub_ind == IdentificationData.CDMA.BTS[i].PN &&
                                                        restech.ResultItems[k].station_identifier_from_radio == IdentificationData.CDMA.BTS[i].GCID)
                                                    {
                                                        if (sd.MeasStart < restech.ResultItems[k].spec_data.MeasStart || sd.MeasStart == DateTime.MinValue)
        ...
        private void GpsConnect()
        {
            try
            {
                ...
                if (DeviceType == DeviceType.Tsme || DeviceType == DeviceType.Tsmw)
                {
                ...
                else if (DeviceType == DeviceType.Tsme6)
                {
                    settings.enGnssMode = GnssMode.Type.GPS_GLONASS_GALILEO;// settings.enSatNavSystem = SatNavSystem.Type.GPS; //settings.enGnssMode = GnssMode.Type.GPS;// 
                    settings.enGPSMessageFormat = GPSMessageFormat.Type.VICOM_GPS_FORMAT_NMEA;//.VICOM_GPS_FORMAT_UBLOX;
                    settings.enResetMode = ResetMode.Type.NONE;
                    settings.deadReckoningSettings.enState = SDeadReckoningSettings.State.Type.DISABLED;
                }
        ...
        private void RfPowerScanConnect()
        {
            try
            {
                ...
                rSSweepSettings.sSpectrumSettings.eBandwidthType = SSpectrumSettings.Bandwidth.Type.CLEAR_SPECTRUM;
        ...
        private SSweepSettings RfPowerScanSetSweepSettings()
        {
            ...
            try
            {
                ...
                sss.sSpectrumSettings.eBandwidthType = SSpectrumSettings.Bandwidth.Type.CLEAR_SPECTRUM;
                ...
                try
                {
                    ...
                    foreach (SGPSMessage sgpsm in pData.ListOfMessages)
                    {
                        if (sgpsm.enMessageFormat == GPSMessageFormat.Type.VICOM_GPS_FORMAT_NMEA)
                        {
                            ...
                            MainWindow.gps.ReadNMEAData(mes);
                                        ...
                                        if (IdentificationData.GSM.BTS[i].FreqIndex == pr.dwFrequencyIndex && pr.dwIndicatorOfSCHInfo == IdentificationData.GSM.BTS[i].IndSCHInfo)
                                        {
                                            ...
                                            if (MainWindow.gps.Altitude != 0 && MainWindow.gps.LatitudeDecimal != 0 && MainWindow.gps.LongitudeDecimal != 0)
                                            {
                                                ...
                                                if ((IdentificationData.GSM.BTS[i].level_results.Count == 0 ||
                                                    ...
                                                    IdentificationData.GSM.BTS[i].level_results[IdentificationData.GSM.BTS[i].level_results.Count - 1].level_dbm != IdentificationData.GSM.BTS[i].Power) ||//скорее перестраховка
