                ...
                foreach (MeasSubTaskStation SubTaskStation in SubTask.MeasSubTaskStations)
                {
...

                    {
                        ...
                        if (Type == "New")
                        {
                            ...
                            if (task.Status == "O") { MTSDR.status = "O"; } else { MTSDR.status = "N"; }
                            ...
                            if (task.MeasTimeParamList.PerInterval != null) { MTSDR.PerInterval = task.MeasTimeParamList.PerInterval.Value; } else { MTSDR.PerInterval = 10; } // 10 секунд на измерения по умолчанию 
