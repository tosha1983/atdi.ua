    ...
    public class LoadProtocols
    {
                                 ...
                                 DateTime? permissionStop)
        {
            ...
            try
            {
                ...
                builderProtocols.Select(c => c.DateMeasDay,
                                        ...
                                        c => c.STATION_EXTENDED.StationTxChannel,
                                        c => c.STATION_EXTENDED.StationRxChannel,
                ...
                queryExecuter.Fetch(builderProtocols, readerProtocols =>
                {
                    while (readerProtocols.Read())
                    {
                        ...
                        protocols.StationExtended.StationTxChannel = readerProtocols.GetValue(c => c.STATION_EXTENDED.StationTxChannel);
                        protocols.StationExtended.StationRxChannel = readerProtocols.GetValue(c => c.STATION_EXTENDED.StationRxChannel);
            ...
            try
            {
                ...
                queryExecuter.Fetch(builderProtocols, readerProtocols =>
                {
                    while (readerProtocols.Read())
                    {
                        ...
                        var channelsTx = readerProtocols.GetValue(c => c.STATION_EXTENDED.StationTxChannel);
                        if ((channelsTx != null) && (channelsTx.Length > 0))
                        {
                            protocols.StationTxChannel = string.Join(";", channelsTx);
                        }
                        var channelsRx = readerProtocols.GetValue(c => c.STATION_EXTENDED.StationRxChannel);
                        if ((channelsRx != null) && (channelsRx.Length > 0))
                        {
                            protocols.StationRxChannel = string.Join(";", channelsRx);
                        }
                        ...
                        var rxFreq = readerProtocols.GetValue(c => c.STATION_EXTENDED.StationRxFreq);
                        if ((rxFreq != null) && (rxFreq.Length > 0))
                        {
                            protocols.StationRxFreq = string.Join(";", rxFreq);
                        }
                                ...
                                )
        {
            ...
            try
            {
                ...
                {
                    builderProtocols.Where(c => c.TimeMeas, ConditionOperator.GreaterEqual, new DateTimeOffset(new DateTime(DateStart.Value.Year, DateStart.Value.Month, DateStart.Value.Day, 0, 0, 0, 1)));
                    builderProtocols.Where(c => c.TimeMeas, ConditionOperator.LessEqual, new DateTimeOffset(new DateTime(DateStop.Value.Year, DateStop.Value.Month, DateStop.Value.Day, 23, 59, 59, 999)));
                }
