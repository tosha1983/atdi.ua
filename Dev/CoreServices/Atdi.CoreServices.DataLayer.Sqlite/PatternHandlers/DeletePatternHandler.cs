...
{
    class DeletePatternHandler : LoggedObject, IQueryPatternHandler
    {
        public DeletePatternHandler(ILogger logger) : base(logger)
        {
        }
            ...
            switch (pattern.Result.Kind)
            {
                ...
                case EngineExecutionResultKind.RowsAffected:
                    pattern.AsResult<EngineExecutionRowsAffectedResult>()
                        .RowsAffected = executor.ExecuteNonQuery(command);
                    return;
        ...
        private void BuildIteration(PS.DeletePattern pattern, BuildingContex context)
        {
            for (var i = 0; i < pattern.Expressions.Length; i++)
            {
        ...
        private void BuildExpression(PS.DeleteExpression expression, BuildingContex context)
        {
            ...
            if (expression.Joins != null && expression.Joins.Length > 0)
            {
                for (var i = 0; i < expression.Joins.Length; i++)
                {
                    ...
                    var sqlJion = context.BuildJoinExpression(join);
                    sqlJoins.Add(sqlJion);
                }
            }
