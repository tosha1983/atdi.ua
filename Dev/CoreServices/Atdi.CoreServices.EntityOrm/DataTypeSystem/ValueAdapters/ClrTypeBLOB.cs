...
{
    class ClrTypeBLOB : ValueAdapter<object, byte[]>
    {
        public ClrTypeBLOB(IDataTypeMetadata dataTypeMetadata, ILogger logger) : base(dataTypeMetadata, logger)
        {
        }
        ...
        {
            var store = ValueReader.ReadAsBLOB(dataReader, ordinal);
            var value = store.Deserialize<object>();
            return value;
        }
